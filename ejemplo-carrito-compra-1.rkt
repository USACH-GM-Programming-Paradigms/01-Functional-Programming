...
;; Ejemplo: (producto "libro" 9000)
(define (producto nombre precio)
...
; alternativa, usando lambda, es lo mismo
;(define producto
...
(define (addItem cc prod cantidad)
  (let ([i (item prod cantidad)])
    (updateItem cc i)))

(define (updateItem cc i)
  (if (null? cc)
      (cons i null)
...
(define (addItem2 cc prod cantidad)
...
;versión full declarativa de la función anterior
(define (totalAPagarCarritoDecl cc)
  (apply + (map (lambda (i) (* (getCantidad i) (getPrecio (getProducto i))))
                cc)))
