...
; input: (rotate-right (list 1 2 3 4))
; output: 4 1 2 3
;define (rotate-right lst)
 ; (if (null? lst) null
...
(define (count symbol lst)
  (cond [(null? lst) 0]
        [(eqv? (car lst) symbol)
         (+ 1 (count symbol (cdr lst)))]
        [else (count symbol (cdr lst))]))

; (histogram (list 1 2 3))
...
; Ref: https://docs.racket-lang.org/reference/pairs.html#%28def._%28%28lib._racket%2Fprivate%2Flist..rkt%29._sort%29%29
...
(count '(1 2 3 4) '('(1 2 3 4) '(4 5 6)))

(define (image w h . pixels)
  (list w h pixels))
